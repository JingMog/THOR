...
def unpad_dataproto(data: 'DataProto', pad_size, sample_n=1):
    if pad_size != 0:
        pad_size = pad_size * sample_n
        data = data[:-pad_size]
    return data
...
class DataProto:
    ...
    def check_consistency(self):
        ...
        if self.batch is not None and len(self.non_tensor_batch) != 0:
            ...
            for key, val in self.non_tensor_batch.items():
                ...
                assert isinstance(val, np.ndarray), 'data in the non_tensor_batch must be a numpy.array with dtype=object'
                assert val.shape[
    ...
    def chunk(self, chunks: int) -> List['DataProto']:
        ...
        """
        # qkchang modified for filter code result
        if False:
