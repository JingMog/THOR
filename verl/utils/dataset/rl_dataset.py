...
class RLHFDataset(Dataset):
    ...
    def _read_files_and_tokenize(self):
        ...
        elif self.template_type=="text_only":
            self.dataframe = self.dataframe[self.dataframe.apply(lambda doc: len(tokenizer.encode(text_only.replace("[PROMPT]", doc[prompt_key][1]['content']))) <= self.max_prompt_length-10, axis=1)]
        elif self.template_type=="qwen3_text_only":
            self.dataframe = self.dataframe[self.dataframe.apply(lambda doc: len(tokenizer.encode(qwen3_text_only.replace("[PROMPT]", doc[prompt_key][1]['content']))) <= self.max_prompt_length-10, axis=1)]
        elif self.template_type=="qwen3_long_tir":
            self.dataframe = self.dataframe[self.dataframe.apply(lambda doc: len(tokenizer.encode(long_cot_prompt.replace("[PROMPT]", doc[prompt_key][1]['content']))) <= self.max_prompt_length-10, axis=1)]
        elif self.template_type == "long_cot":
            self.dataframe = self.dataframe[self.dataframe.apply(lambda doc: len(tokenizer.encode(long_cot_prompt.replace("[PROMPT]", doc[prompt_key][1]['content']))) <= self.max_prompt_length-10, axis=1)]
        else:
            raise ValueError
    ...
    def __getitem__(self, item):
        ...
        if self.template_type=="default":
            ...
            prompt_with_chat_template = text_only.replace("[PROMPT]", chat[1]['content'])
        elif self.template_type=="qwen3_long_tir":
            prompt_with_chat_template = long_cot_prompt.replace("[PROMPT]", chat[1]['content'])
        elif self.template_type=="qwen3_text_only":
            prompt_with_chat_template = qwen3_text_only.replace("[PROMPT]", chat[1]['content'])
        else:
            raise ValueError
