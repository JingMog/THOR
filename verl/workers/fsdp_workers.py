...
class ActorRolloutRefWorker(Worker):
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def generate_sequences(self, prompts: DataProto, val=False):
        ...
        with self.rollout_sharding_manager:
            ...
            output = self.rollout.generate_sequences(prompts=prompts, val=val)
...
class CriticWorker(Worker):
    ...
    def compute_values(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data=data)
            values, code_interpreter_values_diff = self.critic.compute_values(data=data)
            output = DataProto.from_dict(tensors={'values': values, 'code_interpreter_values_diff': code_interpreter_values_diff})
            output = self.ulysses_sharding_manager.postprocess_data(data=output)
